...
for key, values in dict_final.items():
    ...
    filename = f"Resultado_menos_hsa/{key}_resultado_final"
    menos_hsa.to_csv(filename, sep="\t", index=False, header=False)
    print(f"Created {filename}")



"""for files in os.listdir(diretorio_atual):
    filename = os.fsdecode(files)
    if filename.endswith(".asm") or filename.endswith(".tsv"): 
        dicionario_ortologos[filename] = []
        df = pd.read_csv(filename, sep = "\t")
        ortologos = []
        for index, row in df.iterrows():
            ortologos.append(row['protein_mansoni'].split(', '))
        ortologos = flatten_extend(ortologos)
        dicionario_ortologos[filename] = ortologos
        continue
    else:
        continue
for key, values in dicionario_ortologos.items():
    df = pd.DataFrame({key: values})

    filename = f"output/{key}"
